...
After editing Python files (like `cvmfs_extension/routes.py`):

```bash
python -m py_compile cvmfs_extension/__init__.py  # Check single file for syntax errors
```
...
**Python** (in `cvmfs_extension/*.py` files):
...
- Backend processing in `cvmfs_extension/routes.py`
...
- **✅ Do**: `cvmfs_extension/` with underscores, all lowercase
...
```typescript
const PLUGIN_ID = 'cvmfs_extension:plugin';
```
...
export namespace CommandIDs {
  export const openPanel = 'cvmfs_extension:open-panel';
  export const refreshData = 'cvmfs_extension:refresh-data';
}
...
- Backend routes: `cvmfs_extension/routes.py` (class `DataRouteHandler`)
- Frontend logic: `src/` directory
- Python package: `cvmfs_extension/` directory
...
1. **Read the backend first** — Check `cvmfs_extension/routes.py` to understand the existing API contract
...
Create RESTful endpoints in `cvmfs_extension/routes.py`:
...
def setup_route_handlers(web_app):
    ...
    data_route = url_path_join(base_url, "cvmfs_extension", "data")
...
- Define matching endpoint path strings (e.g., `"hello"`, `"get-data"`) in both `cvmfs_extension/routes.py` and `src/api.ts` to ensure routes sync between backend and frontend
...
2. **Write the code** (TypeScript in `src/`, styles in `style/`, Python in `cvmfs_extension/`)
...
5. **Install the extension** (REQUIRED for JupyterLab to recognize it):
   ...
   jupyter server extension enable cvmfs_extension
   ```
...
- **Output**: Creates files in `lib/` and `cvmfs_extension/labextension/`
...
**After editing Python** (files in `cvmfs_extension/`):
...
- Ask user to search for the extension ID (`cvmfs_extension`) to see if it loaded
...
ls -la cvmfs_extension/labextension/  # Should contain bundled extension
```
...
- Backend routes: `cvmfs_extension/routes.py`
- Server extension config: `jupyter-config/server-config/cvmfs_extension.json`
...
**❌ Don't**: Write frontend interfaces without first checking the backend API contract in `cvmfs_extension/routes.py`
...
**❌ Don't**: Use absolute imports like `from cvmfs_extension.routes import ...`
...
- **Plugin ID** (in `src/index.ts`): `'cvmfs_extension:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'cvmfs_extension:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-cvmfs-extension-ClassName`
- **API routes** (in `cvmfs_extension/routes.py`): `url_path_join(base_url, "cvmfs_extension", "endpoint")`
